...
const DEFAULTS = {
  ...
  txtSvcFmt:[
    'Страница :: 120 :: 4 :: ',
    'Сайт :: 200 :: 8 :: берут чаще',
    'Каталог :: 280 :: 10 :: ',
    'Штучный :: 350 :: 13 :: ',
    'Редизайн :: 150 :: 6 :: берут чаще',
    'Бот :: 120 :: 5 :: берут чаще',
    'Мини-приложение :: 180 :: 8 :: ',
    'Бот и приложение :: 260 :: 11 :: ',
    'Приглашение :: 60 :: 3 :: берут чаще',
    'Сайт события :: 120 :: 5 :: '
  ].join('|'),
  /* доп: «название :: цена :: дней» — разделы и пояснения лежат в ADD.
     ...
     («Каталог» и «Мини-приложение»), а не продаётся отдельной строкой */
  txtSvcAdds:[
    /* сайты */
    'Телеграм-бот :: 80 :: 3',
    ...
    'Короткая ссылка :: 20 :: 1 :: берут чаще',
    'Версия для печати :: 30 :: 2',
    /* всегда */
    'Поддержка :: 30 :: 0 :: берут чаще',
  ...
  txtSvcDiffSites:[
    ...
    'Сайт и Штучный :: В «Сайте» экраны собираются из решений, уже проверенных на других проектах. В «Штучном» каждый экран придумывается заново — сцена, анимация, свой характер. Это то, за что сайт запоминают и пересылают друзьям.',
    'Каталог :: Берут, когда разделов и товаров много: карточки живут в одном файле данных, добавить позицию — строчка, а не новая страница. Плюс фильтры, поиск и SEO под каждую карточку.',
  ...
  txtAdvTitle:'Что остаётся у вас',
  ...
  /* Все десять форматов из прайса, той же группировкой: сайты, телеграм,
     события — плюс «пока не знаю» в конце. */
  /* Названия слово в слово как на карточках конструктора (txtSvcFmt).
     Пока они расходились, человек выбирал «Сайт с нуля» там, где только что
     нажал карточку «Сайт», и в заявке приезжали два разных названия одного
     и того же. Последний пункт — не формат, а выход для тех, кто ещё не решил. */
  txtCtKinds:'Страница|Сайт|Каталог|Штучный|Редизайн|Бот|Мини-приложение|Бот и приложение|Приглашение|Сайт события|Пока не знаю',
...
/* Формат в чеке и пункт списка в форме — два независимых списка в твикере,
   поэтому ищем совпадение по словам, а не по индексу: правку одного из них
   индекс переживёт молча и подставит не тот формат.
   Сначала точное совпадение: сейчас списки одинаковые, и по первому слову
   «Сайт события» нашло бы «Сайт» — он в списке раньше и тоже начинается
   с этого слова. */
function svcKindFor(name){
  const kinds = String(cfg.txtCtKinds).split('|').map(s => s.trim());
  const low = String(name).trim().toLowerCase();
  const exact = kinds.find(k => k.toLowerCase() === low);
