...
const DEFAULTS = {
  ...
  /* Куда форма шлёт заявку. Раньше вела на CRM (Hugging Face) — там же и
     ...
     сервера, который нечему засыпать и нечем упереться в квоту. Получил
     заявку — переслал тем же ботом (@aeterna_crm_bot) в личку.
     Пусто или недоступно — форма работает по запасному пути: открывает
     телеграм с уже готовым текстом заявки. */
  txtLeadUrl:'https://black-star-1a91.tochkaq1.workers.dev/',
  txtMenu:'МЕНЮ',
  txtMenuOpen:'ЗАКРЫТЬ',
  ...
  txtSvcFmt:[
    'Страница :: 120 :: 4 :: ',
    'Сайт :: 200 :: 8 :: чаще всего',
    'Многостраничный :: 280 :: 10 :: ',
    'Авторский :: 350 :: 13 :: ',
    'Редизайн :: 150 :: 6 :: чаще всего',
    'Бот :: 120 :: 5 :: чаще всего',
    'Мини-приложение :: 180 :: 8 :: ',
    'Бот и приложение :: 260 :: 11 :: ',
    'Приглашение :: 60 :: 3 :: чаще всего',
    'Сайт события :: 120 :: 5 :: '
  ].join('|'),
  /* доп: «название :: цена :: дней» — разделы и пояснения лежат в ADD.
     Порядок строк обязан совпадать с порядком позиций в ADD.
     «Панель заявок», «Магазин с оплатой», «Оплата в боте» и «Панель
     заказов» отсюда убраны: CRM-панели клиентам больше не делаю, а приём
     оплаты теперь входит в сам формат, где он нужен по смыслу
     («Многостраничный» и «Мини-приложение»), а не продаётся отдельной строкой */
  txtSvcAdds:[
    /* сайты */
    'Телеграм-бот :: 80 :: 3',
    'Тексты под ключ :: 60 :: 2',
    'Логотип и фирстиль :: 50 :: 2',
    'Фото под сайт :: 40 :: 1',
    /* телеграм */
    'Каталог товаров :: 80 :: 3',
    'Рассылки :: 50 :: 2',
    'Сценарии и тексты :: 40 :: 2',
    'Иконка и обложка :: 30 :: 1',
    /* события */
    'Ответы гостей :: 30 :: 1',
    'Карта и дорога :: 20 :: 1',
    'Фото и музыка :: 25 :: 1',
    'Текст приглашения :: 30 :: 1',
    'Короткая ссылка :: 20 :: 1 :: чаще всего',
    'Версия для печати :: 30 :: 2',
    /* всегда */
    'Поддержка :: 30 :: 0 :: чаще всего',
    'Разбор через месяц :: 45 :: 1'
  ].join('|'),
  ...
  txtAdvList:[
    'Быстрая разработка :: Сайт с нуля — до двух недель, а не двух месяцев.',
    'Фокус на конверсию :: Экран ведёт к заявке, а не просто красиво выглядит.',
    '100% адаптивность :: Телефон, планшет, монитор — проверяю каждый.',
  ...
  txtObjList:[
    'Почему так дёшево — не подвох? :: Я в начале пути и цену держу честной под это, а не притворяюсь студией с историей. Работа при этом та же, что и на дорогих сайтах: свой код без конструкторов и шаблонов, панель для правок, замеренная скорость и человек, к которому можно вернуться через полгода. Цены будут расти по мере того, как растёт портфолио — сейчас как раз момент прийти раньше остальных.',
    ...
    'А вы не пропадёте после сдачи? :: Исходники отдаю сразу и целиком — сайт ваш, его может продолжить любой разработчик. Это не угроза мне, а ваша страховка. Поддержка стоит 30 BYN в месяц и означает правки в течение месяца, а не «мы вас записали».',
  ...
  /* Вариант А: не обещания на словах, а то, что видно и проверяется тут же,
     на самом сайте — рядом метки Lighthouse, открытый код, живые ссылки.
     Прежний текст (срок/результат/цена/поддержка) был тем же набором клише,
     что у любой студии, только своими словами.
     Числа тут не пишем нарочно: «7 работ», «3 живых» устареют в тот же день,
     когда добавится восьмая — WORKS в этом же файле их и так считает */
  txtPrList:[
    'Скорость :: у каждой работы свой балл PageSpeed, а не одна цифра на все',
    'Код :: открой консоль, там нет чужого фреймворка или конструктора',
    'Честно :: нигде не написано «10 лет опыта», если это неправда',
    'Живое :: у части работ свой адрес — открой и посмотри, а не только скриншот'
  ].join('|'),
  ...
  txtStkList:[
    ...
    'Адаптивность :: Ровно на телефоне, планшете и мониторе. Проверяю каждый, а не полагаюсь на «оно само подстроится».',
    ...
    'PageSpeed :: Меряю скорость перед сдачей и держу выше 90. Это не обещание, а число, которое вы проверите сами.',
  ...
  /* {n} — число работ подставляется из длины WORKS при сборке, а не
     пишется тут руками: раньше стояло «16», хотя в сетке ниже было 7 —
     число молчаливо разъехалось с тем, что человек мог сам пересчитать
     на этой же странице */
  txtAboutFacts:[
    'Минск :: на связи в своём часовом поясе',
    '{n} работ :: разных задач, разных решений',
  ...
  txtCtKinds:'Одна страница|Сайт с нуля|Каталог или магазин|Анимация или сцена|Редизайн|Бот в телеграме|Мини-приложение|Бот и приложение вместе|Приглашение или сайт события|Пока не знаю',
...
const WORKS = [
  ...
  { n:'КИНОНОЧЬ',      c:'сайты компаний', t:'частный кинотеатр',d:'брутализм с кислотным акцентом',    s:'10 секций',        img:'assets/works/cinemanight.webp',  url:'https://tochkaq1-afk.github.io/cinemanight/',
  ...
  { n:'МОТОАРЕНА',     c:'каталоги',       t:'мототехника',     d:'фильтры, гео-блок, галерея',         s:'JSON-каталог',     img:'assets/works/motoarena.webp',    url:'https://tochkaq1-afk.github.io/motoarena/',
  ...
  { n:'FlowerHome',    c:'магазины',       t:'цветы с доставкой',d:'каталог с 3D-анимацией',            s:'CSS 3D',           img:'assets/works/flowerhome.webp',   url:'https://tochkaq1-afk.github.io/flowerhome/',
...
const FMT = [
  ...
  { g:0, d:'каталог, разделы, много товаров',
    ...
    inc:['Разделы и каталог','Карточки из файла данных','Фильтры и поиск','Корзина и приём оплаты','Панель правки текстов','Показываю, как править самому','Метрика и цели','Скорость 95+','Домен и выкладка','SEO под карточки'] },
  ...
  { g:0, d:'у вас есть сайт, но он устарел',
    ...
    inc:['Разбор нынешнего сайта','Новый дизайн на старых текстах','Перенос содержимого без потерь','Адаптивность и скорость заодно','Домен и выкладка'] },
...
function buildAbout(){
  ...
  abFacts.innerHTML = String(cfg.txtAboutFacts).replace('{n}', WORKS.length)
    .split('|').map((f, i) => {
...
/* Заявка. Сервера пока нет, поэтому кнопка сначала честно пробует его
   ...
   Появится свой бэкенд — поменяется только адрес, остальное останется. */
function leadText(){
...
/* ---------------------------------------------- шапка прячется на спуске
   Листают вниз — шапка уезжает, листают вверх — возвращается. Порог в
   6 пикселей обязателен: без него дрожание тачпада на месте переключало
